    ...
    if request.method == "POST":
        ...
        if(isinstance(transls_json, dict)):
            for key, value in transls_json.items():
                ...
                group, _ = TranlsationGroups.objects.get_or_create(title=group_key, sub_text=group_key)
                translation, _ = Translations.objects.update_or_create(group=group, key=key, value=json_value)
...
class TranslationGroupUdpate(UpdateView):
    ...
    def collect_translation(self, request, row) -> dict:
        ...
        for lang in self.langs:
            value_data[lang.code] = request.POST.get(f"value[{row}][{lang.code}]")
    ...
    def collect_new_translations(self, request) -> list:
        new_objects = []
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            context_data['new_objects'] = self.collect_new_translations(request)
