...
def json_field_validate(value):
    ...
    if value.get(lang.code, '') == '' or value.get(lang.code, '') == None:
...
class Languages(models.Model):
    ...
    def save(self, *args, **kwargs) -> None:
        if self.default:
            ...
            if self.id:
                langs = langs.exclude(id=self.id)

            for lng in langs:
                lng.default = False
                lng.save()
        
        cache.set('langs', Languages.objects.filter(active=True).order_by('-default'))
...
class BaseModel(models.Model):
    ...
    def save(self, *args, **kwargs):  # new
        ...
        try:
            instace = self.__class__.objects.get(id=self.id)
        except:
            instace = None
        ...
        if update_slug:
            if not lng:
                lng = Languages.objects.filter(active=True, default=True).first()
